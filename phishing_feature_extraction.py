...
computer_raw_data=pd.read_csv("new_phishing_urls.txt",header=None,names=['urls'])
print(computer_raw_data.head(10))

rows = len(computer_raw_data)
print(rows)


#Making class of feature Extraction
cd = None
...
# making series for features to be stored
protocol = []
domain = []
path = []
having_ip = []
...
#Extracting and storing features from class feature extraction by using functions
...
#Assigining labels '0' for legitimate term
label = []
for i in range(0,rows):
    label.append(1)
...
#storing the features in csv format
data.to_csv("extracted_phishing_urls.csv",index=False,encoding='UTF-8')
